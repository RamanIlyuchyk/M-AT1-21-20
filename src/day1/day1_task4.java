...
public class day1_task4 {
    public static void main(String[] args) {
        ...
        System.out.print("\nМассив, где каждые 2 соседних элемента поменяли местами:");
        swapPairsOfElements(array);
        Arrays.stream(array).boxed().collect(Collectors.toList()).forEach(x -> System.out.print(" " + x));
    }
    ...
    public static void swapPairsOfElements(int[] array) {
        for (int i = 0; i < array.length - 1; i += 2) {
            int elementForSwap = array[i];
            array[i] = array[i + 1];
            array[i + 1] = elementForSwap;
        }
    }
}